...
interface UKDownloadBrochureButtonProps {
  course: Course;
...
export default function UKDownloadBrochureButton({
  course,
  variant = "solid",
  className = "",
  uploadedBrochureUrl,
}: UKDownloadBrochureButtonProps) {
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const [{ pdf }, { default: UKCourseBrochurePDF }] = await Promise.all([
        import("@react-pdf/renderer"),
        import("./UKCourseBrochurePDF"),
      ]);

      const blob = await pdf(<UKCourseBrochurePDF course={course} />).toBlob();
