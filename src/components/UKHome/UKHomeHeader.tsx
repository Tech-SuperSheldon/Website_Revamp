...
const ACADEMY_MENU = academyMenu("uk");
...
export function Header({ stacked = false }: { stacked?: boolean }) {
  ...
  return (
    <>
      {/*
        ── `stacked` (used on /uk/home): sticky + in normal flow, so it sits
              ...
              link.name === "Academies" ? (
                <div key={link.name} className="flex w-full max-w-[280px] flex-col items-center">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xl font-medium text-gray-800 hover:text-blue-600 transition-colors"
                  >
                    {link.name}
                    <ChevronDown
                      size={20}
