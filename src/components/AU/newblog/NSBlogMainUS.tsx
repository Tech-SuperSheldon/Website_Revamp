      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        {blogs.length > 0 && (
          <motion.div
            ...
            className="mb-20"
          >
            <Link href={`/au/blogs/${blogs[0].id}`} className="group relative block w-full aspect-21/9 rounded-[2.5rem] overflow-hidden shadow-2xl bg-gray-100">
              <Image
                src={blogs[0].image}
                alt={blogs[0].title}
                fill
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {blogs.slice(1).map((blog, idx) => (
            ...
            >
              <Link href={`/au/blogs/${blog.id}`} className="group h-full flex flex-col">
