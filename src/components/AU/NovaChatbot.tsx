...
export default function NovaChatbot({ mobileHidden = false }: { mobileHidden?: boolean }) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hi! I'm **Nova**, your SuperSheldon assistant 👋\n\nI can help you with:\n• Our three academies — School Readiness, Exam Readiness (NAPLAN, ICAS, Selective, ACER, GATE, ATAR, UCAT) and Skill Academy\n• Booking a free demo session\n\nWhat would you like to know?",
    },
  ]);
  ...
  const handleBookingFlow = async (userText: string) => {
    ...
    if (bookingStep === "grade") {
      ...
      addMessage({
        role: "assistant",
        content: "What's your **mobile number**? (e.g. +61412345678)",
      });
      return;
    }
