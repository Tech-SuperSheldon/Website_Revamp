...
//   {
//     video: "/videos/vid2.mp4",
//     heading: "1:1 Online Sessions with Expert Teachers",
...
export default function VideoChoose({ themeColor = "#f97316" }) {
  ...
  // Variants ensuring entry always comes from the right (100) if direction is positive
  const slideVariants = {
    enter: (direction) => ({
      x: direction > 0 ? 100 : -100, // Enter from Right if going next
      opacity: 0,
    }),
    center: {
      zIndex: 1,
    ...
    exit: (direction) => ({
      zIndex: 0,
      x: direction < 0 ? 100 : -100, // Exit to Left if going next
      opacity: 0,
    }),
  };
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-6">
        ...
        {/* Grid Layout - White Box Removed */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          
          {/* Video Visualizer (LHS) - 4:3 Aspect Ratio */}
          <div className="w-full max-w-[640px] mx-auto lg:mx-0 relative aspect-[4/3] rounded-2xl overflow-hidden bg-black shadow-xl">
            <AnimatePresence initial={false} custom={direction} mode="popLayout">
              <motion.video
                key={slides[index].video}
                custom={direction}
              ...
              {slides.map((_, i) => {
                ...
                return (
                  <button
                    key={i}
                    onClick={() => paginate(i)}
                    className="focus:outline-none group py-2" // Added padding for easier clicking
                    aria-label={`Go to slide ${i + 1}`}
                  >
                    <motion.div
                      initial={false}
                      animate={{
                        width: isActive ? 48 : 10, // Active: bar (48px), Inactive: dot (10px)
                        backgroundColor: isActive ? themeColor : "#d1d5db", // gray-300 for inactive
                      }}
